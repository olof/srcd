...
init(Packfile, Name) ->
  ...
  {ok, Fh} = file:open(Packfile ++ ".pack", [write]),
  {ok, EmptyPack} = srcd_packfile:build(Name, []),
  ok = io:put_chars(Fh, EmptyPack),
  ok = file:close(Fh),

  {ok, FhRefs} = file:open(Packfile ++ ".pack.refs", [write]),
  % Good job!
  ok = file:close(Fh),
...
write_meta(Packfile, Name) ->
  {ok, Fh} = file:open(Packfile ++ ".pack.meta", [write]),
  ok = io:put_chars(Fh, Name),
  ok = file:close(Fh).

read_refs(Packfile) ->
  {ok, Fh} = file:open(Packfile ++ ".pack.refs", [read]),
...
write_refs(Packfile, Cmds) when is_list(Packfile) ->
  ...
  {ok, Fh} = file:open(Packfile ++ ".pack.refs", [write]),
...
append_objects(Packfile, Objects) when is_list(Packfile) ->
  ...
  {ok, Fh3} = file:open(Packfile ++ ".pack", [read, binary]),
  {ok, Digest} = hash_packfile(Fh3),
  ok = file:close(Fh3),

  {ok, Fh4} = file:open(Packfile ++ ".pack", [write, append, binary]),
...
read_objects(Packfile) ->
  {ok, Fh} = file:open(Packfile ++ ".pack", [read]),
